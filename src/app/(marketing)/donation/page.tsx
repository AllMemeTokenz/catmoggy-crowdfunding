    ...
    try {
      ...
      if (Array.isArray(response.data)) {
        ...
        if (donationsArray.length === 0 && Object.keys(response.data).length > 0) {
          donationsArray = Object.values(response.data) as ApiDonationItem[];
        }
      }

      // ✅ Then: check if it's still empty
      if (donationsArray.length === 0) {
        console.log('No Data Available!');
        throw new Error('No donations data available');
      }

      // ✅ Make sure it's still an array
